...
This stacked bar plot compares the first six categories that contain most games with two game types. Since the expansion for base-game and board game expansion are similar, the light blue which represents the board game expansion takes over the category of expansion for base-game. For the other categories, the number of board games is much more than the board game expansion in Card Games and War games, and the number of board games is also slightly more in fantasy, fighting, and science fiction. According to visualization 2, board games are much more than board game expansion, which correlates in visualization 3 that we can see board game outweighs board game expansion in most of the categories, except for Expansion for Base-game. 

We assume that maybe the game producers prefer to produce the board games since they might cost less or contain a higher profit. However, in some categories, the companies need to add new game areas, to create a better game experience, such as fantasy, fighting, and science fiction, which will be relatively evenly distributed into two types of game. In other categories of games, such as card games and war games, the necessity to combine with other elements is relatively low, which the amount of board games is much more than the board game expansion.
...
This scatter plot shows the relationship between the average rating and average complexity of these board games. They have a relatively moderate positive linear correlation (r = 0.5228426), but there are a few outliers. The average rating of the user towards the games is expected to find a positive difference in scores on average when comparing to the average complexity of the games that differs in scores by 1. When the average rating is between 5.0 and 7.5, more games are concentrating on average complexity between 1 and 4. When average complexity reaches 4 and 5, there are much fewer data points shown in the graph, inferring that there is much less rating for the games that have higher average complexity. So, we can also assume that there are fewer high complexity games. 
